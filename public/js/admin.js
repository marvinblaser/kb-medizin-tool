...
async function deleteRole(slug) { if(!confirm('Supprimer ?')) return; const res = await fetch(`/api/admin/roles/${slug}`, { method: 'DELETE' }); if(res.ok) { loadRoles(); showNotification('Supprimé', 'success'); } }
...
async function openUserModal(id=null){ 
    const form=document.getElementById('user-form'); form.reset(); document.getElementById('user-id').value=''; document.getElementById('password-group').style.display='block'; document.getElementById('user-password').required=true; document.getElementById('user-modal-title').innerHTML='<i class="fas fa-user-plus"></i> Ajouter'; await loadRoles();
...
async function saveUser(){ 
    const id=document.getElementById('user-id').value; const formData=new FormData(); formData.append('name',document.getElementById('user-name').value); formData.append('email',document.getElementById('user-email').value); formData.append('role',document.getElementById('user-role').value); formData.append('phone',document.getElementById('user-phone').value); formData.append('is_active',document.getElementById('user-active').checked?1:0);
    const fileInput=document.getElementById('user-photo'); if(fileInput.files.length>0) formData.append('photo',fileInput.files[0]);
    if(!id){const pwd=document.getElementById('user-password').value; if(pwd.length<6)return showNotification('MDP trop court','error'); formData.append('password',pwd);}
    const res=await fetch(id?`/api/admin/users/${id}`:'/api/admin/users',{method:id?'PUT':'POST',body:formData}); if(res.ok){showNotification('Enregistré','success');closeUserModal();loadUsers();} else{const e=await res.json();showNotification(e.error,'error');}
}
async function deleteUser(id){ if(id===window.currentUserId)return showNotification('Impossible','warning'); if(!confirm('Supprimer ?'))return; await fetch(`/api/admin/users/${id}`,{method:'DELETE'}); loadUsers();showNotification('Supprimé','success');}
...
async function deleteEquipment(id) { 
    ...
    if(res.ok){loadEquipment();showNotification('Supprimé','success');}else{showNotification('Erreur','error');} 
}

// ========== MATERIALS ==========
async function loadMaterials(){const r=await fetch('/api/admin/materials');const d=await r.json();document.getElementById('materials-tbody').innerHTML=d.map(m=>`<tr><td><strong>${escapeHtml(m.name)}</strong></td><td><code>${escapeHtml(m.product_code)}</code></td><td>${parseFloat(m.unit_price).toFixed(2)}</td><td style="text-align:right"><div class="table-actions"><button class="btn-icon-sm btn-icon-primary" onclick="openMaterialModal(${m.id})"><i class="fas fa-pen"></i></button><button class="btn-icon-sm btn-icon-danger" onclick="deleteMaterial(${m.id})"><i class="fas fa-trash"></i></button></div></td></tr>`).join('');}
...
async function deleteMaterial(id){if(!confirm('Supprimer ?'))return;const res=await fetch(`/api/admin/materials/${id}`,{method:'DELETE'});if(res.ok){loadMaterials();showNotification('Supprimé','success');}else{showNotification('Erreur','error');}}
