...
function buildLoansFormatBar(targetId) {
  const colors = [
    { label: 'Défaut', value: '' },
  ...
  const btn = `background:none;border:1px solid var(--border-primary);border-radius:3px;
    cursor:pointer;padding:2px 7px;font-size:11px;font-family:inherit;
    color:var(--text-secondary);display:inline-flex;align-items:center;gap:3px;`;
  return `
    <div style="display:flex;align-items:center;gap:5px;padding:5px 8px;
      background:var(--bg-secondary);border:1px solid var(--border-primary);
      border-bottom:none;border-radius:3px 3px 0 0;">
      <button type="button" style="${btn}" onclick="applyLoansFormat('hilite','${targetId}')">
      ...
      ${colors.map(c => `
        <button type="button" title="${c.label}" onclick="applyLoansFormat('color','${targetId}','${c.value}')"
          style="width:16px;height:16px;border-radius:50%;cursor:pointer;padding:0;flex-shrink:0;
            background:${c.value || 'var(--text-primary)'};border:2px solid ${c.value || 'var(--border-primary)'};">
        </button>`).join('')}
      <div style="width:1px;height:16px;background:var(--border-primary);margin:0 2px;"></div>
      <button type="button" style="${btn}font-size:10px;" onclick="applyLoansFormat('clear','${targetId}')">
        <i class="fas fa-times" style="font-size:9px;"></i>&nbsp;Effacer
      </button>
    </div>
    <div id="${targetId}" contenteditable="true"
      style="width:100%;min-height:70px;padding:9px 10px;
...
window.applyLoansFormat = function(type, targetId, value) {
  const el = document.getElementById(targetId);
  if (!el) return;
  el.focus();
  if (type === 'hilite') document.execCommand('hiliteColor', false, '#fef08a');
  else if (type === 'color') {
          ...
          <div style="margin-top:4px;display:flex;flex-wrap:wrap;gap:3px;">
            ${l.device_owner === 'Fournisseur'
              ...
              : l.device_owner === 'KB Med'
                ...
                : ''}
            ${l.rma_id ? `
              <span style="background:rgba(139,92,246,0.12);color:#8b5cf6;border:1px solid rgba(139,92,246,0.3);
                font-size:9px;font-weight:700;padding:1px 6px;border-radius:2px;cursor:pointer;"
                onclick="event.stopPropagation(); window.location.href='/rmas.html?open=${l.rma_id}'"
                title="${escHtml(l.rma_number || '')}">
                <i class="fas fa-link" style="font-size:8px;margin-right:2px;"></i>${escHtml(l.rma_number || '#' + l.rma_id)}
              </span>` : ''}
          </div>
        </td>
        <td>${l.cabinet_name
          ? `<i class="fas fa-hospital" style="opacity:0.35;font-size:10px;margin-right:4px;"></i>${escHtml(l.cabinet_name)}`
               ...
               ${history.map(h => {
                 ...
                 return `
                   <tr style="border-bottom:1px solid var(--border-primary);">
                     <td style="padding:10px 12px;font-size:var(--text-sm);font-weight:600;">${escHtml(h.cabinet_name || '—')}</td>
                     ...
                     <td style="padding:10px 12px;font-size:var(--text-xs);color:var(--text-tertiary);">${days}j${isOpen ? ' (en cours)' : ''}</td>
                     <td style="padding:10px 12px;">
                       ${h.return_condition
                         ? `<span style="font-size:10px;font-weight:600;color:${h.return_condition === 'Bon état' ? '#10b981' : h.return_condition === 'Endommagé' ? 'var(--color-danger)' : 'var(--color-warning)'};">${escHtml(h.return_condition)}</span>`
...
window.openLoanModal = function(id = null) {
  ...
  setTimeout(() => {
    ...
    ['loan-client', 'loan-device', 'loan-rma-link'].forEach(selId => {
      ...
      if (tsLoanInstances[selId]) { try { tsLoanInstances[selId].destroy(); } catch {} }
      if (!el.disabled) {
        tsLoanInstances[selId] = new TomSelect(`#${selId}`, cfg);
        if (savedValue) tsLoanInstances[selId].setValue(savedValue, true);
      }
    });
  }, 50);
};
...
window.openLoanDetail = function(id) {
  ...
  document.getElementById('detail-body').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--text-tertiary);margin-bottom:10px;letter-spacing:0.06em;">Appareil</div>
          ...
          ${loan.device_owner ? `
            <div style="margin-top:6px;">
              <span style="font-size:9px;font-weight:700;padding:1px 6px;border-radius:2px;
                background:${loan.device_owner === 'Fournisseur' ? 'rgba(245,158,11,0.12)' : 'rgba(44,90,160,0.10)'};
                color:${loan.device_owner === 'Fournisseur' ? '#f59e0b' : 'var(--color-primary)'};
                border:1px solid ${loan.device_owner === 'Fournisseur' ? 'rgba(245,158,11,0.3)' : 'rgba(44,90,160,0.25)'};">
                ${escHtml(loan.device_owner)}
              </span>
            </div>` : ''}
        </div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--text-tertiary);margin-bottom:10px;letter-spacing:0.06em;">Client</div>
        <div style="background:var(--bg-secondary);border:1px solid var(--border-primary);padding:12px;">
          ${loan.cabinet_name
    ...
    <div style="margin-top:14px;background:var(--bg-secondary);border:1px solid var(--border-primary);padding:12px;">
      ${loan.reason ? `
        <div style="margin-bottom:8px;">
          <span style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--text-tertiary);">Motif</span>
          <div style="font-size:var(--text-sm);margin-top:3px;">${escHtml(loan.reason)}</div>
        </div>` : ''}
      ${loan.notes ? `
        <div>
          <span style="font-size:10px;font-weight:700;text-transform:uppercase;color:var(--text-tertiary);">Notes</span>
          <div style="font-size:var(--text-sm);margin-top:3px;">${sanitizeHtml(loan.notes)}</div>
        </div>` : ''}
    </div>` : ''}
    ...
    <div style="margin-top:14px;background:rgba(139,92,246,0.06);border:1px solid rgba(139,92,246,0.25);padding:12px;">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
        ...
        <a href="/rmas.html?open=${loan.rma_id}" style="font-size:var(--text-xs);color:#8b5cf6;text-decoration:none;
