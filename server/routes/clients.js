...
router.post('/import', requireAuth, upload.single('file'), (req, res) => {
    ...
    } catch (error) { res.status(500).json({ error: "Erreur lecture Excel" }); }
});
...
router.get('/', requireAuth, (req, res) => {
    ...
    if (sortBy && ['cabinet_name', 'city', 'appointment_at', 'created_at'].includes(sortBy)) order = `c.${sortBy} ${sortOrder === 'desc' ? 'DESC' : 'ASC'}`;
...
router.put('/bulk-update', requireAuth, (req, res) => {
    ...
    if (!ids || ids.length === 0) return res.status(400).json({ error: "Aucune sélection." });
    let sql = "";
...
router.put('/:clientId/equipment/:eqId', requireAuth, (req, res) => {
    ...
    db.run(`UPDATE client_equipment SET equipment_id=?, serial_number=?, installed_at=?, last_maintenance_date=?, maintenance_interval=?, next_maintenance_date=?, location=?, notes=?, is_secondary=? WHERE id=? AND client_id=?`, 
    [equipment_id, serial_number, installed_at, last_maintenance_date, maintenance_interval, nextDate, location, notes, is_secondary ? 1 : 0, req.params.eqId, req.params.clientId], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        res.json({ success: true });
    });
});
